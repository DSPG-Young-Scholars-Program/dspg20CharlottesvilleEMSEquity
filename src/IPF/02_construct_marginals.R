...
age_sex_marginals <- age_by_sex %>%
  ...
  mutate(total = estimate_total_estimate,
         females = estimate_total_female_estimate,
         males = estimate_total_male_estimate) %>%
  group_by(GEOID, total, females, males) %>%
  rowwise() %>%
...
colnames(race_marginals) <- c("GEOID", "total", "white", "black", "am_ind", "asian", "hawaiian_pac_isl", "other", "multiple")
