...
ggplot(plot_data) +
  ...
  facet_grid(agency_name ~ .) +
  scale_x_datetime(date_breaks = "2 hour", labels = date_format("%H:%M")) +
...
incident_points <- new_ems_data %>%
  ...
  select(incident_date, response_incident_number, scene_gps_latitude, scene_gps_longitude, scene_incident_street_address) %>%
  distinct() %>%
  filter(!is.na(response_incident_number)) %>%
  mutate(across(.cols = c("scene_gps_longitude", "scene_gps_latitude"), .fns = as.numeric))

# leaflet(incident_points) %>%
#   addTiles() %>%
#   addCircleMarkers(lng = ~scene_gps_longitude,
