...
combine_secondary <- fill_deduped %>% 
  group_by(response_incident_number, response_ems_unit_call_sign) %>% 
  mutate(situation_provider_secondary_impression_description_and_code = paste0(unique(situation_provider_secondary_impression_description_and_code), collapse = "|")) %>% 
  ungroup() %>% 
  filter(!duplicated(.))
...
```{r}
# readr::write_csv(remaining_duplicates, here("data", "working", "remaining_duplicates.csv"))
```
