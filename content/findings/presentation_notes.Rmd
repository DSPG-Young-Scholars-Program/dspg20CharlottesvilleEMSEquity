...
## Has Covid Exasperbated Existing Healthcare Disparities in Charlottesville and Albemarle County Emergency Medical Services?
...
county_tracts_sp <- st_read(here("data", "original", "albemarle_tracts.geojson"),
                            quiet = TRUE)
...
volume_change %>%
  ...
  addPolygons(color = "#444444", weight = 0.5, smoothFactor = 0.5,
              ...
              label = ~map(glue("{NAME}<br/>
                                Percent Change in Daily Call Volume: {round(change_ratio, 2)}"), htmltools::HTML)) %>%
  addLegend("bottomright", pal = color_scale, values = ~change_ratio,
            title = htmltools::HTML("Percent Change in Daily Call Volume:<br/>After March 15th"),
            opacity = .8,
