...
According to the Johns Hopkins Center for Systems Science and Engineering, as of June 9, 2020 (the most current date in our data), there were 258 confirmed cases and 8 deaths in Albemarle County, and 141 confirmed cases and 3 deaths in Charlottesville City.[^JHU] While experts expect that actual case counts may be as much as 10 times higher than the confirmed case counts[^NYT] it remains unclear whether this figure is reprsentative of the most severe cases, which are those most likely to appear in the EMS dataset. Given the uncertainty surrounding the total number of cases in the area as well as the proportion of cases that may be present in our dataset, it was unclear how effective an analysis of COVID-symptoms would be. Still, we were curious to see whether any trends would emerge, and in doing so hoped to learn about the potential for future use of EMS data in evaluating the state of the pandemic.
...
_Add tracts to map, potentially add another map with stricter COVID cutoff_
_add city border to plot_
...
leaflet() %>%
  ...
  addPolygons(data = post_period_summed,
              ...
              group = "In 2020") %>%
  addPolylines(data = city_border, color="black", weight = 8, opacity = 0.8) %>%
...
_Add in some more viz of demographic breakdown for COVID cases_
_figure out why formatting is weird_
...
ems %>%
  ...
  count() %>%
  filter(covid1 == 1) %>%
  inner_join(pop_estimates_by_race, by = "race") %>%
  mutate(prop_of_pop = n / pop_estimate) %>%
  ggplot(., aes(x = race_short, y = prop_of_pop, fill = on_or_after_march_15th_2020)) +
    geom_bar(stat = "identity", position = "dodge") + 
    geom_text(aes(label = round(prop_of_pop, digits = 2)), position=position_dodge(width=0.9), vjust=-0.25) + 
...
r17 <- which.max(transposed[17,])

col <- seq_len(ncol(transposed))

transposed[1,] <- transposed[1,] %>% kableExtra::cell_spec(bold = col == r1)
transposed[2,] <- transposed[2,] %>% kableExtra::cell_spec(bold = col == r2)
transposed[3,] <- transposed[3,] %>% kableExtra::cell_spec(bold = col == r3)
transposed[4,] <- transposed[4,] %>% kableExtra::cell_spec(bold = col == r4)
transposed[5,] <- transposed[5,] %>% kableExtra::cell_spec(bold = col == r5)
transposed[6,] <- transposed[6,] %>% kableExtra::cell_spec(bold = col == r6)
transposed[7,] <- transposed[7,] %>% kableExtra::cell_spec(bold = col == r7)
transposed[8,] <- transposed[8,] %>% kableExtra::cell_spec(bold = col == r8)
transposed[9,] <- transposed[9,] %>% kableExtra::cell_spec(bold = col == r9)
transposed[10,] <- transposed[10,] %>% kableExtra::cell_spec(bold = col == r10)
transposed[11,] <- transposed[11,] %>% kableExtra::cell_spec(bold = col == r11)
transposed[12,] <- transposed[12,] %>% kableExtra::cell_spec(bold = col == r12)
transposed[13,] <- transposed[13,] %>% kableExtra::cell_spec(bold = col == r13)
transposed[14,] <- transposed[14,] %>% kableExtra::cell_spec(bold = col == r14)
transposed[15,] <- transposed[15,] %>% kableExtra::cell_spec(bold = col == r15)
transposed[16,] <- transposed[16,] %>% kableExtra::cell_spec(bold = col == r16)
transposed[17,] <- transposed[17,] %>% kableExtra::cell_spec(bold = col == r17)

transposed %>% kable(booktabs = TRUE, escape = FALSE, digits = 2)
```
