...
leaflet(width = "100%") %>%
  ...
  addPolygons(data = city_border,
              ...
              fill = NA, options = pathOptions(pane = "border")) %>%
  # addLayersControl(baseGroups = c("Black", "White")) %>%
  addLegend("bottomright", pal = pal, values = county_neighborhoods_sp$med_age)
```
...
Call volume is not dropping uniformly across Charlottesville and Albemarle. The following map makes it clear that areas immediately surrounding the University have experienced the most drastic drop in call volume, which is almost certainly related to the departure of students from campus after the beginning of the pandemic (unsurprisingly, this is accompanied by a noticable drop in alcohol-related incidents). Similarly, areas surrounding downtown have also seen a more noticable decrease, likely related to the fewer interactions taking place in these public spaces after many bars and restaurants closed in the early stages of the pandemic.

Importantly, the spatial patterns in call volume drop appear to be driven primarily by these peripheral factors. It is not immediately apparent that any particular residential neighborhoods have experienced substantially different call volume drops.
