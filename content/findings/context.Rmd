...
neighborhoods_sp <- st_read(here("data", "working", "neighborhood_demographics.geojson"), quiet = TRUE) %>%
  st_transform(crs = 4326) %>%
  select(NAME, n, prop_white, prop_black, prop_asian)
...
leaflet(width = "100%") %>%
  ...
  addLegend("bottomright", pal = pal, values = county_neighborhoods_sp$prop_black)
```
...
_In terms of income, we see areas of lower income coincide with areas with higher black populations and can also observe the same trend with higher poverty rates. Income and poverty rates seem to coincide as well with areas of higher income to have lower poverty rates._

The distribution of various demographic characteristics reveals some notable patterns in the characteristics of EMS patients. First, black and white patients represent the overwhelming majority of EMS patients. Second, the age distribution of patients varies quite a bit across racial categories. Whites and blacks using EMS services tend to be older than other racial groups. This is likely a result of the presence of the University of Virginia, which has more Asian student representation than the city and county as a whole. Similar patterns may be at play in the slight bump at young ages for black patients as well. While this pattern is less noticable for whites, this is likely a result of a generally larger white population muting out the effects of the University demographics. Given the increased risks of COVID-19 for older adults, the higher representation of blacks and whites at older ages is worth noting when considering groups at highest risk in Charlottesville.
...
char_race_gen_na %>%
  ...
  geom_split_violin(alpha= 1, color = "#fafafa")+
  ...
  #scale_color_manual(values = cbPalette, name = "Sex", labels = c("Female", "Male"))+
  geom_jitter(data =  sample_n(char_race_gen_na,7500), position = position_jitter(.025), size = .7, alpha =.25, show.legend = FALSE) +
...
Call volume is not dropping uniformly across Charlottesville and Albemarle, either. The following map makes it clear that areas immediately surrounding the University have experienced the most drastic drop in call volume, which is almost certainly related to the departure of students from campus after the beginning of the pandemic (unsurprisingly, this is accompanied by a noticable drop in alcohol-related incidents in the EMS data).

Simialrly, areas surrounding downtown have also seen a more noticable decrease, again likely because of the fewer interactions taking place in these public spaces after many bars and restaurants closed in the early stages of the pandemic.

Importantly, the spatial patterns in call volume drop appear to be driven primarily by these peripheral factors. It is not immediatley apparent that particular neighborhoods (which may be a proxy for particular demographic groups) have experienced wildly different call volume drops.
...
color_scale <- colorBin(color_pal, c(0, 1.5), c(0.1, 0.35, 0.65, 0.90, 1.10, 1.3))
...
volume_change %>%
  ...
  addPolygons(color = "#444444", weight = 0.5, smoothFactor = 0.5,
              ...
              label = ~map(glue("<strong>{NAME}</strong><br/>
                                Ratio of Daily Incident Rate: {round(change_ratio, 2)}"), htmltools::HTML)) %>%
  addPolygons(data = city_border,
              color = "#222222", weight = 3, smoothFactor = 0.5,
              fill = NA) %>%
  addLegend("bottomright", pal = color_scale, values = ~change_ratio,
            title = htmltools::HTML("Ratio of Daily Incidents:<br/>Post COVID-19 to Pre COVID-19"),
            opacity = .8,
...
The causes of mortality are not uniform across racial categories, either. Black patients are more than 2.5 times as likely to die from diseases of the blood and twice as likely to die from complications arising during the perinatal period. This is consistent with the widely-reported health disaprities between black and white women when it comes to maternal morbidity.[^CDC] White individuals, in contrast, are more likely to die from skin conditions and, notably, respiratory illnesses. The circulatory disorders mentioned earlier appear to affect blacks and whites at roughly the same rate. 

However, it is worth noting that these represent very general categories of death. As a more nuanced understanding of the interactions between underlying health conditions and COVID-19 develops, it may be valuable to explore these mortality trends in more depth.
...
While the VDH data provided important context for understanding the health disparities in the region, we did not notice an overall increase in mortality since the beginning of the COVID-19 pandemic, suggesting that the pandemic may have to play out longer before we are able to assess its overall effects on mortality in the region. Additionally, the VDH data lacked information on patient race for many of the deaths that have occurred since the beginning of the COVID-19 pandemic, preventing us from exploring the interaction between death counts, race, and the onset of COVID-19.
